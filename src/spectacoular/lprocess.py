...
class CalibHelper(ac.TimeOut, BaseSpectacoular):
    ...
    calibstd = Float(
        0.5, desc="allowed standard deviation of calibration values in buffer"
    )
    ...
    def result(self, num):
        ...
        for temp in self.source.result(num):
            ...
            # print(calibmask)
            if (calibmask.max() == self.buffer_size) and (
                calibmask.sum() == self.buffer_size
            ):
            ...
            for i in np.arange(self.num_channels):
                self.calibfactor[i] = self.to_pa(self.magnitude) / self.to_pa(
                    float(self.calibdata[i, 0])
                )
            yield temp
            ...
            if self.channels:
                if isinstance(self.source, ac.MaskedTimeSamples):
                    sig = self.source.data[
                        self.source.start : self.source.stop, self.channels
                    ].sum(1)
                else:
