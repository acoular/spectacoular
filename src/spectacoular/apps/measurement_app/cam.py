"""Camera overlay component for the measurement app."""
...
XCAM = (-0.5, -0.375, 1.0, 0.75)


class CameraComponent(BaseSpectacoular):
    """Bokeh-backed camera image component."""

    doc = Instance(bokeh.document.Document)
    active = Bool(default_value=False, desc='Whether to use the camera')
    ...
    flip_horizontal = Bool(default_value=False, desc='Flip the camera image horizontally')
    ...
    def _camera_index_callback(self, _attr, _old, new):
    ...
    def _start_camera_stream(self, cds):
        ...
        self._vc_stream.set(3, width)
        self._vc_stream.set(4, height)
        ...
        view = img.view(dtype=np.uint8).reshape((height, width, 4))[::-1, ::-1]
        view[:, :, 3] = 255
    ...
    def _global_alpha_callback(self, _attr, _old, new):
        ...
        for renderer in self.figure.renderers:
            if (
                renderer is not self._glyph_renderer
                and hasattr(renderer, 'global_alpha')
                and renderer.global_alpha > new
            ):
