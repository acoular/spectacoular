...
class SinusControl(MeasurementControl):
    def __init__(
        ...
        **kwargs,
        ...
        if config_name is not None:
            value = config_name
        config_options = ["None"] + [
            f.name for f in Path(config_dir).iterdir() if f.is_file()
        ]

        self.select_setting = Select(
            title="Select Settings:", value=value, options=config_options, width=100
        )
        self.reload_settings_options = Button(label="↻", disabled=False, width=30)
        self.settings_button = Button(
            label="Load Setting", disabled=False, button_type="warning"
        )
        self.level_select = Select(
            title="Select Level to load:",
            value="AnalogInput",
            options=[
                (None, "All"),
    ...
    def create_buffer_bar(self):
        ...
        self.buffer_bar.hbar(
            y="y", height=0.9, left=0, right="filling", source=self.buffer_cds
        )
    ...
    def get_widgets(self):
        return column(
            [
                ...
                Spacer(height=100),
                row(
                    column(Spacer(height=15), self.reload_settings_options),
                    self.select_setting,
                ),
                self.level_select,
    ...
    def set_sinus_callbacks(self):
        self.reload_settings_options.on_click(
            self.update_select_settings_options_callback
        )
        self.settings_button.on_click(self.settings_callback)

    def get_analog_input_tab(self):
        set_settings_button = Button(
            label="Set Analog Input Settings", button_type="warning"
        )
        ...
        return Panel(
            child=layout([[set_settings_button], [analog_input_widgets]]),
            title="Analog Input Settings",
        )

    def get_analog_output_tab(self):
        set_settings_button = Button(
            label="Set Analog Output Settings", button_type="warning"
        )
        ...
        return Panel(
            child=layout([[set_settings_button], [analog_output_widgets]]),
            title="Analog Output Settings",
        )

    def get_device_tab(self):
        set_settings_button = Button(
            label="Set PCI Device Settings", button_type="warning"
        )
        ...
        for pci in self.device.pci:
            ...
            trait_widget_mapper.update(
                {
                    k: Select
                    for k in pci._settable_attr
                    if k not in ["BlockCount", "SyncWithDevices"]
                }
            )
            ...
            devices_widgets.append(
                list(
                    sp.get_widgets(
                        pci,
                        trait_widget_mapper=trait_widget_mapper,
                        trait_widget_args=mics_trait_widget_args,
                    ).values()
                )
            )
        return Panel(
            child=layout([[set_settings_button], [devices_widgets]]),
            title="PCI Device Settings",
        )

    def get_adc_to_dac_tab(self):
        set_settings_button = Button(
            label="Set ADC to DAC Settings", button_type="warning"
        )
        ...
        return Panel(
            child=layout([[set_settings_button], [adc_to_dac_widgets]]),
            title="ADC to DAC Settings",
        )
    ...
    def _update_teds_data(self, event):
        self.teds_cds.data = {
            ...
            "serial": [
                c.TEDSData.get("SensorSerNo", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
            "sensitivity": [
                c.TEDSData.get("SensorSensitivity", "")
                if hasattr(c, "TEDSData")
                else ""
                for c in self.device.analog_inputs
            ],
            "wiredata": [
                c.TEDSData.get("1_Wire_Data", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
            "calibdate": [
                c.TEDSData.get("CalibDate", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
            "calibperiod": [
                c.TEDSData.get("CalibPeriod", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
            "chipserial": [
                c.TEDSData.get("ChipSerNo", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
            "manufacturer": [
                c.TEDSData.get("Manufacturer", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
            "sensorversion": [
                c.TEDSData.get("SensorVersion", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
            "tedstemp": [
                c.TEDSData.get("TEDS_Template", "") if hasattr(c, "TEDSData") else ""
                for c in self.device.analog_inputs
            ],
        }
    ...
    def get_teds_tab(self):
        ...
        self._update_teds_data(None)
        teds_table = DataTable(
            source=self.teds_cds, columns=tedscolumns, sizing_mode="stretch_width"
        )
        download_button = Button(
            label="📥", button_type="warning", sizing_mode="fixed", width=30, height=30
        )
        download_js = CustomJS(
            args=dict(source=self.teds_cds),
            code="""
        ...
        return Panel(
            child=layout([[download_button, teds_table]], sizing_mode="stretch_both"),
            title="TEDS Data",
        )
