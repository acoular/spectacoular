...
def build_calib_data(orchestrator, name_input, source_select, notes):
    ...
    calib_data["Date"] = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    for channel_num in sorted(orchestrator.channels.keys()):
        if float(orchestrator.channels[channel_num].calib_value_final) != 0:
            ...
            if str(orchestrator.channels[channel_num].unit) == "dB":
                ...
                channel["CalibLevel"]["Value"] = pa_to_dB(float(orchestrator.channels[channel_num].calib.referenceMagnitude))
            else:
                channel["CalibFactor"]["Unit"] = str(orchestrator.channels[channel_num].unit)+"/V"
                channel["CalibLevel"]["Value"] = float(orchestrator.channels[channel_num].calib.referenceMagnitude)
...
def save(path_input, notes, source_select, orchestrator, logger):
    ...
    if not path.parent.exists():
        logger.debug(f"The directory does not exist: {str(path.parent)}")
        return
    ...
    logger.debug(f"{str(path.name)} saved in: {str(path.parent)}")
