...
def server_doc(doc):
    ...
    mic_layout.presenter.cdsource.on_change(
        "data", update
    )  # automatically re-calc if set

    # create Slider widget to choose Frequency of PSF
    psfFreqSlider = Slider(
        title="Frequency [Hz]", value=psf.freq, start=10.0, end=20000.0
    )
    psf.set_widgets(
        **{"freq": psfFreqSlider}
    )  # set from file attribute with select widget
    ...
    def change_pos(attr, old, new):
        ...
        grid_index = np.array(
            [np.ravel_multi_index(rg.index(*source_pos[:2]), rg.shape)]
        )
    ...
    srcRenderer = psf_figure.scatter(
        marker="cross", x="x", y="y", size=10, fill_alpha=0.8, source=src_pos
    )
    ...
    psfTab = Panel(
        child=layout([list(psf.get_widgets().values())]), title="Point-Spread Function"
    )
    ...
    # Steering Tab
    stTab = Panel(child=layout([list(st.get_widgets().values())]), title="Steering")
