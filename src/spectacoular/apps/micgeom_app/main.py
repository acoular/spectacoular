...
from bokeh.models import ColorBar, ColumnDataSource, LinearColorMapper, PointDrawTool, Spacer, Tabs
from bokeh.models import TabPanel as Panel
from bokeh.models.widgets import NumberEditor, NumberFormatter, Slider, TableColumn
...
rg = sp.RectGrid(x_min=-0.5, x_max=0.5, y_min=-0.5, y_max=0.5, z=0.5, increment=0.02)
...
def server_doc(doc):
    """Populate a Bokeh document for the microphone geometry app."""
    ...
    psf_freq_slider = Slider(title='Frequency [Hz]', value=psf.freq, start=10.0, end=20000.0)
    psf.set_widgets(freq=psf_freq_slider)  # set from file attribute with select widget

    # location for which the PSF is calculated
    src_pos = ColumnDataSource(data={'x': [0], 'y': [0]})

    def change_pos(_attr, _old, _new):
        source_pos = (
            src_pos.data['x'][0],
            src_pos.data['y'][0],
            rg.z,
        )  # (x,y,z), will be snapped to grid
    ...
    psf_figure.image(
        ...
        dw='dw',
        dh='dh',
        source=psf_presenter.cdsource,
        color_mapper=cm,
    )
    psf_figure.add_layout(
        ColorBar(
            color_mapper=cm,
            location=(0, 0),
            title='Lp/dB',
    ...
    src_renderer = psf_figure.scatter(marker='cross', x='x', y='y', size=10, fill_alpha=0.8, source=src_pos)
    marktool = PointDrawTool(renderers=[src_renderer], num_objects=1)
    ...
    psf_tab = Panel(child=layout([list(psf.get_widgets().values())]), title='Point-Spread Function')
    ...
    grid_tab = Panel(
        child=layout(
            [
                [rg_widgets['x_min'], rg_widgets['x_max']],
                [rg_widgets['y_min'], rg_widgets['y_max']],
                [rg_widgets['z'], rg_widgets['increment']],
                [rg_widgets['nxsteps'], rg_widgets['nysteps']],
            ],
            width=TAB_WIDTH,
        ),
        title='Grid',
    ...
    # Steering Tab
    st_tab = Panel(child=layout([list(st.get_widgets().values())]), title='Steering')
    ...
    control = layout(
        [
            [Spacer(width=400, height=10)],
            [psf_freq_slider],
            [Tabs(tabs=[mg_tab, grid_tab, psf_tab, st_tab])],
        ],
        width=TAB_WIDTH,
    )
