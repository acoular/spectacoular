...
from numpy import array, newaxis, array_equal, concatenate, stack
...
ALLOWED_WIDGET_TRAIT_MAPPINGS = {
    NumericInput: NUMERIC_TYPES
    + (TraitCompound, Any, Delegate, Union),  # (Trait,Property,Delegate)
    Toggle: (Bool,) + (TraitCompound, Any, Delegate, Union),
    Select: (Enum, TraitEnum, Map, TraitMap, BaseStr, BaseFile, Union)
    + NUMERIC_TYPES,  # Numeric types and Str types should also be allowed here, to further use the set_widgets method with predefined options
...
def validate_mapping_is_allowed(obj, traitname, widgetType):
    ...
    if allowed_trait_types:
        is_allowed_instance = any(
            [
                isinstance(given_trait_type, allowed_type)
                for allowed_type in allowed_trait_types
            ]
        )
...
def get_widgets(self, trait_widget_mapper={}, trait_widget_args={}):
    ...
    * **Custom View:** :meth:`~spectacoular.factory.get_widgets` is called by a :class:`~spectacoular.factory.BaseSpectacoular`
    ...
    widgetdict = {}
    if not hasattr(
        self, "trait_widget_mapper"
    ):  # in case of classes that are not part of Spectacoular
        if (
            not trait_widget_mapper
        ):  # if no trait_widget_mapper defined, take the default
            for traitname in self.editable_traits():
                traittype = self.trait(traitname).trait_type
                defaultWidgetType = DEFAULT_TRAIT_WIDGET_MAPPINGS.get(
                    traittype.__class__
                )
                if defaultWidgetType:
                    widgetMapper = widget_mapper_factory(
                        self, traitname, defaultWidgetType
                    )
...
class SelectMapper(TraitWidgetMapper):
    ...
    def _validate_options(self, options):
        ...
        if len(typeset) > 1:
            raise ValueError(
                "Can only handle options of the same type for widgets of type Select."
            )
...
class SliderMapper(TraitWidgetMapper):
    ...
    def set_widget(self, widget):
        ...
        traitvalue = self.widget.value
        if (
            not self.traitvaluetype == self.traitvaluetype
        ):  # checks if casting is necessary
            traitvalue = self.traitvaluetype(
                traitvalue
            )  # cast to the correct traittype
        self._set_traitvalue(traitvalue)  # set traitvalue to widgetvalue
...
class DataTableMapper(TraitWidgetMapper):
        ...
        if self.widget.editable:
            ...
            if dtype in NUMERIC_TYPES:
                ...
                if dtype is None:
                    warn(
                        (
                            f"Undefined dtype of trait {self.traitname} which is linked to an editable DataTable widget."
    ...
    def _set_widgetvalue(self, traitvalue, widget_property="data"):
        ...
        if isinstance(self.traittype, (List, Tuple)):
            ...
            if dim > 1:
                ...
                new_data = {
                    self.widget.columns[i].field: traitvalue[:, i][:, newaxis]
                    for i in range(traitvalue.shape[1])
                }
            else:
                new_data = {self.widget.columns[0].field: traitvalue}
    ...
    def create_trait_setter_func(self):
        ...
        if isinstance(self.traittype, List):
            ...
            def callback(attr, old, new):
                ...
                new_traitvalue = self._cds_to_numpy_array_transform(
                    column_values, self.transposed
                )
    ...
    def _cds_to_numpy_array_transform(self, column_values, transposed):
        ...
        if (
            column_values[0].ndim == 1 and len(column_values) == 1
        ):  # mapps to arrays with -> (n,) shapes
...
class MultiSelectMapper(TraitWidgetMapper):
    ...
    def __init__(self, obj, traitname):
        ...
        self.traitname = traitname
        self.traittype = obj.trait(traitname).trait_type
        self.item_dtype = obj.trait(
            traitname
        ).trait_type.item_trait.trait_type.fast_validate[1]
        self.traitdescription = obj.trait(traitname).desc
        try:
            self.traitvalue = getattr(obj, traitname)
        except AttributeError:  # in case of Delegate
