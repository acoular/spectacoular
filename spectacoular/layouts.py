...
class MicGeomComponent(BaseSpectacoular):
    ...
    def update_mic_size(self, attr, old, new):
        if (
            type(self.glyph.size) is not str
    ...
    def _add_point_draw(self, event):
        existing_tools = [
            ...
            if isinstance(tool, bokeh.models.PointDrawTool)
        ]
        ...
        if self.allow_point_draw and self._glyph_renderer is not None:
            draw_tool = bokeh.models.PointDrawTool(
                renderers=[self._glyph_renderer], empty_value=0.0
            )
        ...
        if self.presenter is not None:
            if not hasattr(self.presenter.source, "get_widgets"):
                if "pos_total" in self.mics_trait_widget_mapper.keys():
                    if (
                        self.mics_trait_widget_args.get("pos_total") is None
                    ) or self.mics_trait_widget_args["pos_total"].get(
                        "source"
                    ) is not None:
                        self.mics_trait_widget_args["pos_total"].update(
                            {"source": self.presenter.cdsource}
                        )
                widgets = get_widgets(
                ...
                self.presenter.source.trait_widget_mapper.update(
                    self.mics_trait_widget_mapper
                )
                self.presenter.source.trait_widget_args.update(
                    self.mics_trait_widget_args
                )
                if "pos_total" in self.presenter.source.trait_widget_mapper.keys():
                    if (
                        self.presenter.source.trait_widget_args.get("pos_total") is None
                    ) or self.presenter.source.trait_widget_args["pos_total"].get(
                        "source"
                    ) is None:
                        self.presenter.source.trait_widget_args["pos_total"].update(
                            {"source": self.presenter.cdsource}
                        )
