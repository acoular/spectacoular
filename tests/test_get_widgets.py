...
def get_all_classes(hastraits_only=False, module="acoular"):
    ...
    for module_info in pkgutil.walk_packages(package.__path__, package.__name__ + "."):
        ...
        for _, cls in inspect.getmembers(module, inspect.isclass):
            ...
            if cls.__module__ == module_info.name and (
                not hastraits_only or issubclass(cls, HasTraits)
            ):
                classes.append(cls)
    return classes
...
def test_get_widgets(cls):
    ...
    assert len(widgets_default) > 0, (
        f"get_widgets returned an empty dict for {cls.__name__} with default mapping"
    )
